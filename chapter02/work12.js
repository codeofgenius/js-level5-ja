...
以下のプログラムは、オブジェクトTaxCalculatorの
メソッドcalculateを一度変数に代入したため、thisの
参照が外れてしまっています。

変数calculateに対して、bindメソッドを使い、thisが
常にTaxCalculatorを指すように固定した新しい関数を
作成してください。

その新しく作成された関数を呼び出し、引数として0.1を
渡してください。

最終的な計算結果をconsole.log等で表示し、実行結果の
ように表示してください。

bindメソッドの呼び出し部分以外（既存のオブジェクト定義など）を
修正してはいけません。
